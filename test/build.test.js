...
describe('Build an App', function(){
  ...
  it('errors if no config', function(done){
    let config = {path: '.'};
    let testResponse = function(error){
      if(error) {
        return done();
      } else {
        return done(new Error("Error not thrown without config specified"));
      }
    };
    return rallyAppBuilder.build(config, testResponse);
  });
